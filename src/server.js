...
app.post('/agendamentos', [
  ...
  try {
    ...
    console.log(`${data_agendamento}T${hora_inicio}`,horaInicioDate, `${data_agendamento}T${hora_fim}`,horaFimDate)
    ...
    const novoAgendamento = await prisma.agendamento.create({
      data: {
        ...
        hora_inicio: `${data_agendamento}T${hora_inicio}.000Z`,
        hora_fim: `${data_agendamento}T${hora_fim}.000Z`,
        status: 'pendente',
...
app.get('/usuarios/:cpf', async (req, res) => {
  ...
  try {
      const usuario = await prisma.usuario.findUnique({
          where: { cpf: BigInt(cpf) }
      });

      if (usuario) {
          res.json({ superuser: usuario.superuser });
      } else {
